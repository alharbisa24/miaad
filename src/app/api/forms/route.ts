...
export async function POST(req: Request) {
  try {
    ...
    if (error instanceof Error) {
      return NextResponse.json({ error: "حدث خطا غير متوقع", details: error.message }, { status: 500 });
    }
  
    return NextResponse.json({ error: "حدث خطا غير متوقع" }, { status: 500 });
  }
}

const UpdateFormSchema = z.object({
    formId: z.string().min(1, "id النموذج مطلوب"),
  ...
  export async function PUT(req: Request) {
    try {
      ...
      if (!parseResult.success) {
        return NextResponse.json(
          { error: "خطا في البيانات المدخلة", details: parseResult.error.issues },
          { status: 400 }
        );
      }
